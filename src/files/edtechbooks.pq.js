$(document).ready(function () {
  ...
  //   if (!$(this).attr("clicked")) {
  //     $(this).attr("clicked", 1);
  //     var obj = new xAPIObject(
  //       "survey",
  //       $(this).parent().attr("data-survey-id")
  //     );
  //     var result = new xAPIResult(
  //       "chosen",
  //       $(this).attr("data-survey-order-number"),
  //       $(this).text()
  //     );
  ...
  //         .attr(
  //           "data-xapi-id",
  //           new xAPIStatement("answered", obj, null, result).store()
  //         );
  //     } else {
  //       new xAPIStatement("answered", obj, null, result).store(
  //         $(this).parent().attr("data-xapi-id")
  //       );
  //     }
  //   }
  // });
  ...
  $(".answerKey > .card-header").click(function () {
    ...
    if (disallow_tracking != true)
      new xAPIStatement($verb, obj, null, result).store();
  });
  ...
  $(".text-entry").on("change keyup paste", function () {
    ...
    if (!$(this).attr("data-xapi-id")) {
      $(this).attr(
        "data-xapi-id",
        new xAPIStatement("entered", obj, null, result).store()
      );
    } else {
      new xAPIStatement("entered", obj, null, result).store(
        $(this).attr("data-xapi-id")
      );
    }
  });
});
